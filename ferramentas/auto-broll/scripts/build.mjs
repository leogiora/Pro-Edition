/*
 * Build do painel. esbuild empacota src/ui/main.ts em dist/main.js e o HTML e
 * o CSS sao copiados ao lado.
 *
 * `premierepro` e `uxp` ficam de fora do bundle de proposito: sao modulos que
 * o host injeta em tempo de execucao. O codigo os pega por `require()` direto,
 * que o esbuild nao toca no formato iife.
 */
...
await build({
  ...
  bundle: true,
  // O host injeta estes em tempo de execucao; empacotar quebraria.
 ...
 *
 * O UXP resolve `href`/`src` relativos a partir da raiz do plugin, nao da
 * pasta do HTML. Como o `main` do manifest e `dist/index.html`, qualquer
 * referencia a "main.js" ou "styles.css" e procurada em `raiz/` e nao existe —
 * e o carregamento falha em silencio, sem erro no log. Embutir elimina o
 * problema em vez de depender de acertar o caminho.
 */
